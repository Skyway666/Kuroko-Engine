...
void ModuleUI::DrawObjectInspectorTab()
{
	...
	if (selected_obj)
	{
		...
		if (ImGui::Button("Delete"))
			App->scene->game_objs_to_delete.push_back(selected_obj);
...
void ModuleUI::DrawPrimitivesTab() 
{
	...
	if (ImGui::Button("Add cube"))
	{
		...
		if (!App->scene->game_objects.empty())
			App->scene->game_objs_to_delete.push_back(App->scene->game_objects.front()); // Just for asignment 1
		App->scene->game_objects.push_back(cube);
	}
	ImGui::SameLine();
	if (ImGui::Button("Add plane"))
	{
		...
		if (!App->scene->game_objects.empty())
			App->scene->game_objs_to_delete.push_back(App->scene->game_objects.front());// Just for asignment 1
		App->scene->game_objects.push_back(plane);
	}
	if (ImGui::Button("Add sphere")) {
		...
		if (!App->scene->game_objects.empty())
			App->scene->game_objs_to_delete.push_back(App->scene->game_objects.front());// Just for asignment 1
		App->scene->game_objects.push_back(sphere);
	}
	ImGui::SameLine();
	if (ImGui::Button("Add cylinder")) {
		...
		if(!App->scene->game_objects.empty())
			App->scene->game_objs_to_delete.push_back(App->scene->game_objects.front());// Just for asignment 1
		App->scene->game_objects.push_back(cylinder);
	}
...
void ModuleUI::DrawTimeControl()
{
	...
	if (ImGui::ImageButton((void*)ui_textures[PLAY]->getGLid(), ImVec2(w, h), ImVec2(0,0), ImVec2(1, 1), 0, ImVec4(0.0f, 0.7f, 0.7f, App->scene->game_state == PLAYING ? 1.0f : 0.0f)))
		App->scene->Play();

	ImGui::SameLine();
	ui_textures[PAUSE]->getSize(w, h);
	if(ImGui::ImageButton((void*)ui_textures[PAUSE]->getGLid(), ImVec2(w, h), ImVec2(0, 0), ImVec2(1, 1), 0, ImVec4(0.0f, 0.7f, 0.7f, App->scene->game_state == PAUSED ? 1.0f : 0.0f)))
		App->scene->Pause();

	ImGui::SameLine();
	ui_textures[STOP]->getSize(w, h);
	if (ImGui::ImageButton((void*)ui_textures[STOP]->getGLid(), ImVec2(w, h), ImVec2(0, 0), ImVec2(1, 1), 0, ImVec4(0.0f, 0.7f, 0.7f, App->scene->game_state == STOPPED ? 1.0f : 0.0f)))
		App->scene->Stop();
